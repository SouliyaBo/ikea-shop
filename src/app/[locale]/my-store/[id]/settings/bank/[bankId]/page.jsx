...
export default function StoreBankEdit({ params }) {
	const { id } = params;
	...
	useEffect(() => {
		get(
			// `${USER}/${id}`,
			`${process.env.NEXT_PUBLIC_API_LINK}/v1/api/shop-bank?shop=${id}`,
			ACCESS_TOKEN,
			setLoading,
			(data) => {
	...
	const handleSubmit = async (values, resetForm) => {
		const prepareData = {
			...
			bankLogo:
				bankInLaos.find((bank) => bank.bankNameLa === values.name)?.img ||
				userBank?.bankLogo,
			shop: id,
		};
		...
		try {
			if (userBank) {
				...
				localStorage.setItem(PHONE, storeDetail?.phone);
				requestOTP(storeDetail?.phone);
			} else {
				await create(
					...
					(data) => {
						...
						toast({
							title: "ສໍາເລັດແລ້ວ",
							description: "ການເພີ່ມຂໍ້ມູນສໍາເລັດແລ້ວ",
						});
						router.push(`/my-store/${id}/settings`);
					},
					(error) => {
	...
	const requestOTP = (phone) => {
		create(
			...
			(data) => {
				if (data?.error === false) {
					router.push(`/my-store/${id}/verify-otp`);
				}
			},
			(error) => {
				SweetAlert({
					...
					text: "ກະລຸນາກວດສອບ ອິນເຕີເນັດຂອງທ່ານ ແລ້ວລອງໃໝ່ອີກຄັ້ງ.",
				});
			},
		);
	};
										...
										>
											...
											<SelectContent className="w-full">
												{bankInLaos.map((item) => (
													<SelectItem
														key={item.bankCode}
														value={item.bankNameLa}
													>
														{item.bankNameLa} ({item.bankCode})
													</SelectItem>
												))}
											</SelectContent>
										</Select>
