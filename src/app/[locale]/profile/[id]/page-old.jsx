...
export default function Profile({ params }) {
	...
	useEffect(() => {
		if (userToken && id) {
			get(
				...
				() => { },
				(res) => {
					console.log(res.data?.vipLevel, "User");
	...
	console.log("moneyProfit: ", moneyProfit);
	return (
		<div className="flex flex-col w-full h-dvh">
			<section className="relative flex-1 overflow-x-hidden overflow-y-auto bg-gradient-to-t from-gray-100 to-white scroll-smooth">
				...
				<div className="relative">
					<div className={`bg-gradient-to-r ${currentVip.color} px-6 pt-12 pb-20 sm:pb-24 md:pb-28`}>
						...
						<div className="flex justify-center mb-4">
							<div className="relative">
								<Avatar className="w-24 h-24 border-4 shadow-2xl sm:w-28 sm:h-28 md:w-32 md:h-32 border-white/30">
									<AvatarImage src={process.env.NEXT_PUBLIC_MEDIUM_RESIZE + userDetail?.image || "/placeholder.svg"} alt="Profile" />
									<AvatarFallback className="text-xl font-bold sm:text-2xl bg-white/20">{userDetail?.firstName.charAt(0)}</AvatarFallback>
								</Avatar>
								{userVipLevel > 0 && <div
								...
								>
									<VipIcon className="w-4 h-4 text-white sm:w-5 sm:h-5" />
								</div>}
							</div>
						</div>
				...
				<div className="relative z-10 flex-1 min-h-0 px-4 pb-20 -mt-12 sm:-mt-16 md:-mt-20">
					{userDetail?.role === "MEMBER" && userDetail?.memberStatus === "APPROVED" ? (
						<div className="w-full p-3 mb-4 space-y-3 text-center bg-white rounded-lg shadow-md sm:p-4 sm:space-y-4 backdrop-blur-sm">
							<p className="text-lg font-bold text-white"></p>
							<p className="flex items-center justify-center gap-2 text-xl font-bold sm:text-2xl text-green">
								<Wallet />
								{moneyUser?.money ? formatToCurrencyTHB(moneyUser?.money) : 0}
							</p>
							<div className="flex items-center justify-around w-full">
								<p className="flex items-center justify-center gap-2 text-xl font-bold text-green-600 sm:text-2xl">
									<PiggyBank />
