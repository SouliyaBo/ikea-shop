...
export default function ShippingOrders({ params }) {
	const { id } = params;
				...
				{loading ? (
					...
					<div className="flex flex-col items-center justify-center h-[50vh] gap-3">
						...
						<div className="text-center">
							<h3 className="mb-1 text-base font-semibold text-gray-900">ไม่มีคำสั่งซื้อที่รอการจัดส่ง</h3>
							<p className="text-sm text-gray-500">คำสั่งซื้อที่จัดส่งแล้วจะแสดงที่นี่</p>
						</div>
					</div>
				) : (
					<div className="space-y-3">
						{dataBills.map((bill, index) => (
							<div
								...
								className="p-4 transition-all duration-200 bg-white border border-gray-100 shadow-sm cursor-pointer rounded-xl hover:shadow-md hover:border-orange-200"
							>
								{/* Header - Compact */}
								<div className="flex items-center justify-between mb-3">
									<div className="flex items-center gap-2">
										<div className="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-r from-orange-500 to-orange-600">
				...
				<DialogContent className="sm:max-w-[700px] max-h-[85vh] overflow-y-auto">
					<DialogHeader>
						<DialogTitle className="flex items-center gap-2 text-xl">
							...
							รายละเอียดคำสั่งซื้อ (กำลังจัดส่ง)
						</DialogTitle>
					</DialogHeader>
					<div className="grid gap-6 py-4">
						{loading ? (
							...
							<div className="space-y-4">
								...
								<div className="p-4 border border-orange-200 bg-gradient-to-r from-orange-50 to-orange-100 rounded-xl">
									<div className="flex items-center gap-3">
										<div className="flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-r from-orange-500 to-orange-600">
											<Truck className="w-6 h-6 text-white" />
										</div>
										<div>
											<h3 className="font-bold text-orange-900">สินค้ากำลังจัดส่ง</h3>
											<p className="text-sm text-orange-700">
												สินค้าของคุณอยู่ระหว่างการขนส่ง
											</p>
										</div>
									</div>
								</div>
								...
								{ordersInBill.length > 0 && (
									<div className="p-6 border border-orange-100 bg-gradient-to-r from-orange-50 to-orange-100 rounded-xl">
										<div className="flex items-center justify-between">
